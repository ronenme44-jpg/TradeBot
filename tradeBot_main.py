"""Main runtime bot for the reduced public trading-pattern showcase.

The public version keeps the original project shape while limiting the active
strategy surface to ten classical graphical patterns.

Typical local flow:
1. Run generate_shared_candles_stocks.py to create shared_candles.csv.
2. Run this file to read that CSV, detect patterns, and manage one open trade.
...
# Runtime files. These are local state files and are intentionally ignored by Git.
CSV_PATH = _res("shared_candles.csv")
TTP_FEED_JSONL = _res("shared_ttp_feed.jsonl")
CAPITAL_PATH = _res("Bot1_capital.json")
REINFORCEMENT_PATH = _res("Bot1_reinforcement_scores.json")
PENDING_PATTERN_PATH = _res("Bot1_pending_pattern.json")
CLOSED_LOG_PATH = _res("Bot1_closed_trades_log.json")
...
# Alerts are written to JSONL so a reviewer can inspect warnings after the bot runs.
...
def load_candles_from_csv(path=CSV_PATH, limit=390):
    """Read candles from shared_candles.csv and keep the latest rows."""
    ...
    try:
        ...
        if dropped:
            print(f"WARN Dropped {dropped} corrupt candle row(s) from CSV.")
...
def init_file(path, default_value):
    """Create a runtime JSON file when the bot starts for the first time."""
    if not os.path.exists(path):
        if safe_write_json(path, default_value, purpose="init"):
            print(f"Created missing file: {path}")

def initialize_all_files():
    """Ensure every runtime state file exists before the loop starts."""
...
# Fees (fractions) for NET PnL calculations.
# The main bot currently models zero-fee equity simulation.
FEE_BUY  = 0.0
FEE_SELL = 0.0
...
def build_indicator_outcomes(pattern_name: str, confirm_result) -> dict:
    """
    Return a clean indicator-outcome dictionary aligned with the rule schema.

    This function is the bridge between confirmation_functions output and the
    reinforcement key builder.
    """
    expected = []
    try:
...
def enter_trade(df, breakout_idx: int, pattern_type: str, capital_data: dict, live=False, api=None, symbol=SYMBOL, context_key: str | None = None) -> dict | None:
    ...
    if max_qty_allowed < 1 or quantity < 1:
        print(f"Skip Not enough capital to open {side}: capital {available_capital:.2f}$, need >= {per_unit_cost:.2f}$ per unit (incl. fees)")
        return None
    ...
    print(
        ...
        f"Position    : {quantity:g} units  | Notional approx {position_notional:.2f}$\n"
...
def process_signals_and_maybe_enter(
        ...
        if last_val:
            ...
            if ENABLE_VOL_FILTER and vol_ratio is not None:
                if not (VOL_MIN <= vol_ratio <= VOL_MAX):
                    ...
                    if vol_ratio < VOL_MIN:
                        print(f"Skip ({side}) '{name}': Market too quiet | Volatility ~{vol_pct:.2f}%/bar < {min_pct:.2f}% (needs approx {be_pct:.2f}% just to cover fees)")
                    else:
            ...
            if ENABLE_SPREAD_FILTER and spread_ratio is not None:
                if not (SPREAD_MIN <= spread_ratio <= SPREAD_MAX):
                    ...
                    continue
            # Confirmation checks gate weak detections and also feed the reinforcement key.
            ...
            try:
                ...
                if outcomes:
                    ...
                    print(
                        f"Breakdown: {trues} passed / {falses} failed -> "
                        + ", ".join(f"{k}={'PASS' if v else 'FAIL'}" for k, v in outcomes.items())
                    )
                else:
            ...
            if not isinstance(rec, dict):
                # Fresh public clones start with no reinforcement history. Create a
                # neutral record so the bot is runnable before training data exists.
                try:
                    if not isinstance(scores, dict):
                        scores = {}
                    reinforcement_manager.ensure_key(scores, key)
                    reinforcement_manager.save_scores(scores)
                    rec = scores.get(key)
                except Exception as exc:
                    log_alert(
                        "WARN",
                        f"Failed to initialize reinforcement score for key '{key}': {exc}",
                        key="reinforcement_init",
                    )
                    continue
                if not isinstance(rec, dict):
                    log_alert(
                        "WARN",
                        f"Reinforcement score record for key '{key}' is invalid after initialization.",
                        key="reinforcement_record_invalid",
                    )
                    continue
    ...
    print(f"OK Entering with highest score pattern '{pick['name']}'.")
...
if __name__ == "__main__":
    try:
        ...
        # Startup loads the current candle source and prints enough context to
        # confirm the bot is wired to the expected files and pattern set.
        ...
        # Main loop. Each iteration refreshes data, validates session state, and
        # acts only on a new final candle.
        ...
        while True:
            try:
                ...
                data_ok_for_entries = not data_gap_issue
                ...
                if new_candle:
                    ...
                    if not market_open_now:
                        ...
                        if not data_ok_for_entries:
                            print("Entry gate closed due to data gap issues.")
                        else:
